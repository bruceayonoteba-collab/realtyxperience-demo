    ...
    def search_property_knowledge(self, query, max_results=3):
        ...
        try:
            ...
            for _, row in self.property_df.iterrows():
                ...
                if query_lower in question or query_lower in tags or query_lower 
in answer.lower():
                    knowledge.append({
...
class RealtyXperienceAI:
    ...
    def mr_x_response(self, user_question, context=None):
        ...
        # Get knowledge from YOUR CSV database
        knowledge = self.knowledge_base.search_property_knowledge(user_question, 
3)
        ...
        if self.claude_available:
            try:
                ...
                knowledge_context = "Here's what I know from my knowledge 
database:\n\n"
...
{len(properties)} properties available.\n"
                
                system_prompt = """You are Mr. X, a property investment expert 
assistant for RealtyXperience. Use the knowledge database information provided to 
give helpful, detailed answers about rental properties, ROI calculations, 
short-term rentals, and property investment strategies. Be specific and 
professional."""
                
                user_prompt = f"{knowledge_context}{platform_context}\n\nUser 
question: {user_question}\n\nPlease provide a comprehensive answer using the 
knowledge database information above."
...
{item['answer']}\n\n"
                ...
                if context:
                    ...
                    if land_plots:
                        platform_context += f"\n\nCurrent platform has 
{len(land_plots)} land plots available.\n"
                
                system_prompt = """You are Landlord, a land development expert 
assistant for RealtyXperience. Use the knowledge database information provided to 
give helpful, detailed answers about zoning, land development, permits, and land 
investment strategies. Be specific and professional."""
...
knowledge database information above."
    ...
    def _fallback_property_response(self, user_question, context=None):
        ...
        if any(word in message_lower for word in ["invest", "investment", "roi", 
"return"]):
...
Which property or area would you like me to analyze for investment potential?"""
        ...
        elif any(word in message_lower for word in ["find", "search", 
"recommend"]):
...
Let me know your preferences (budget, location, lifestyle needs) and I'll find 
...
def load_initial_properties():
    initial_properties = {
        "Lagos": [
            {
                "id": 1, "name": "Luxury 3BR Apartment", "location": "Ikoyi", 
"bedrooms": 3, "bathrooms": 3, 
                "area_sqm": 150, "rent_monthly": 2500000, "sale_price": 80000000, 
                "amenities": ["Pool", "Gym", "Security", "Generator"], 
                "description": "Modern luxury apartment with sea view", 
"year_built": 2020, 
                "owner_contact": "+234-123-456-789", "status": "active", 
"uploaded_date": "2024-01-15", 
                "demand_score": 8.5, "competition_score": 7.2, 
"seasonal_multiplier": 1.2, 
                "photos": 12, "description_quality": 9, "booking_rate": 0.85, 
"avg_rating": 4.7,
                "owner_id": "owner_1", "available_from": "2024-03-01"
            },
            {
                "id": 2, "name": "Cozy 2BR Flat", "location": "Yaba", "bedrooms": 
2, "bathrooms": 2,
                "area_sqm": 85, "rent_monthly": 800000, "sale_price": 25000000, 
                "amenities": ["Security", "Parking"], "description": "Perfect for 
young professionals", 
                "year_built": 2018, "owner_contact": "+234-123-456-790", "status": 
"active", 
                "uploaded_date": "2024-02-10", "demand_score": 6.8, 
"competition_score": 8.1,
                "seasonal_multiplier": 1.0, "photos": 6, "description_quality": 7, 
                "booking_rate": 0.72, "avg_rating": 4.3, "owner_id": "owner_2", 
"available_from": "2024-03-15"
            },
            {
                "id": 3, "name": "Executive 4BR Duplex", "location": "Lekki Phase 
1", "bedrooms": 4, 
                "bathrooms": 4, "area_sqm": 200, "rent_monthly": 3200000, 
"sale_price": 95000000, 
                "amenities": ["Pool", "BQ", "Security", "Generator", "Garden"], 
                "description": "Spacious family home in prime location", 
"year_built": 2019, 
                "owner_contact": "+234-123-456-791", "status": "active", 
"uploaded_date": "2024-01-20", 
                "demand_score": 9.1, "competition_score": 6.5, 
"seasonal_multiplier": 1.15, 
                "photos": 18, "description_quality": 8.5, "booking_rate": 0.91, 
"avg_rating": 4.8,
        ...
        "Abuja": [
            {
                "id": 4, "name": "Diplomatic 4BR Villa", "location": "Maitama", 
"bedrooms": 4, 
                "bathrooms": 4, "area_sqm": 250, "rent_monthly": 4500000, 
"sale_price": 120000000, 
                "amenities": ["Pool", "BQ", "Security", "Generator", "Garden"], 
                "description": "Premium location for diplomats and executives", 
"year_built": 2019, 
                "owner_contact": "+234-123-456-792", "status": "active", 
"uploaded_date": "2024-01-25", 
                "demand_score": 8.8, "competition_score": 5.9, 
"seasonal_multiplier": 1.3, 
                "photos": 20, "description_quality": 9.2, "booking_rate": 0.88, 
"avg_rating": 4.9,
                "owner_id": "owner_4", "available_from": "2024-03-01"
            },
            {
                "id": 5, "name": "Modern 3BR Apartment", "location": "Wuse II", 
"bedrooms": 3, 
                "bathrooms": 3, "area_sqm": 130, "rent_monthly": 1800000, 
"sale_price": 45000000, 
                "amenities": ["Gym", "Security", "Generator"], 
                "description": "Central business district location", "year_built": 
2020, 
                "owner_contact": "+234-123-456-793", "status": "active", 
"uploaded_date": "2024-02-05", 
                "demand_score": 7.5, "competition_score": 7.8, 
"seasonal_multiplier": 1.1, 
                "photos": 10, "description_quality": 8, "booking_rate": 0.79, 
"avg_rating": 4.5,
...
def load_initial_land():
    initial_land = {
        "Lagos": [
            {
                "id": 201, "name": "Prime Commercial Waterfront Land", "location": 
"Victoria Island", 
                "land_size_sqm": 5000, "price_total": 500000000, "price_per_sqm": 
100000,
                "land_use": "Commercial", "title_document": "C of O", 
"survey_status": "Completed",
                "features": ["Waterfront", "C of O", "Survey Plan", "Corner 
Piece"], 
                "description": "Premium waterfront commercial land perfect for 
high-rise development", 
                "owner_contact": "+234-123-456-800", "status": "active", 
"uploaded_date": "2024-01-15", 
                "demand_score": 9.2, "competition_score": 6.8, 
"seasonal_multiplier": 1.3, 
                "photos": 15, "description_quality": 9.5, "inquiry_rate": 0.92, 
"avg_rating": 4.8,
                "owner_id": "land_owner_1", "available_from": "2024-03-01", 
"zoning": "Commercial",
                "access_road": "Tarred", "topography": "Flat", "utilities": 
["Electricity", "Water", "Internet"]
            },
            {
                "id": 202, "name": "Residential Estate Land", "location": "Lekki 
Phase 2", 
                "land_size_sqm": 2500, "price_total": 75000000, "price_per_sqm": 
30000,
                "land_use": "Residential", "title_document": "Deed of Assignment", 
"survey_status": "Completed",
                "features": ["Gated Estate", "Survey Plan", "Good Drainage"], 
                "description": "Perfect for luxury residential development in 
growing Lekki area", 
                "owner_contact": "+234-123-456-801", "status": "active", 
"uploaded_date": "2024-02-10", 
                "demand_score": 7.8, "competition_score": 7.5, 
"seasonal_multiplier": 1.1, 
                "photos": 8, "description_quality": 8.2, "inquiry_rate": 0.76, 
"avg_rating": 4.5,
                "owner_id": "land_owner_2", "available_from": "2024-03-15", 
"zoning": "Residential",
                "access_road": "Tarred", "topography": "Flat", "utilities": 
["Electricity", "Water"]
            },
            {
                "id": 203, "name": "Industrial Development Land", "location": 
"Ikorodu", 
                "land_size_sqm": 10000, "price_total": 180000000, "price_per_sqm": 
18000,
                "land_use": "Industrial", "title_document": "C of O", 
"survey_status": "Completed",
                "features": ["Large Area", "C of O", "Industrial Zone", "Highway 
Access"], 
                "description": "Excellent for manufacturing and logistics 
operations", 
                "owner_contact": "+234-123-456-802", "status": "active", 
"uploaded_date": "2024-01-20", 
                "demand_score": 8.3, "competition_score": 6.2, 
"seasonal_multiplier": 1.0, 
                "photos": 12, "description_quality": 8.8, "inquiry_rate": 0.83, 
"avg_rating": 4.6,
                "owner_id": "land_owner_3", "available_from": "2024-02-01", 
"zoning": "Industrial",
                "access_road": "Highway", "topography": "Flat", "utilities": 
["Electricity", "Water", "Gas"]
            }
        ],
        "Abuja": [
            {
                "id": 204, "name": "Diplomatic Zone Premium Land", "location": 
"Maitama Extension", 
                "land_size_sqm": 3000, "price_total": 900000000, "price_per_sqm": 
300000,
                "land_use": "Residential", "title_document": "C of O", 
"survey_status": "Completed",
                "features": ["Diplomatic Zone", "C of O", "Corner Piece", "Tarred 
Road"], 
                "description": "Ultra-premium diplomatic zone land for luxury 
development", 
                "owner_contact": "+234-123-456-803", "status": "active", 
"uploaded_date": "2024-01-25", 
                "demand_score": 9.5, "competition_score": 5.8, 
"seasonal_multiplier": 1.4, 
                "photos": 18, "description_quality": 9.8, "inquiry_rate": 0.95, 
"avg_rating": 4.9,
                "owner_id": "land_owner_4", "available_from": "2024-03-01", 
"zoning": "Residential",
                "access_road": "Tarred", "topography": "Flat", "utilities": 
["Electricity", "Water", "Internet", "Gas"]
            },
            {
                "id": 205, "name": "Commercial Business District Land", 
"location": "Central Business District", 
                "land_size_sqm": 4000, "price_total": 600000000, "price_per_sqm": 
150000,
                "land_use": "Commercial", "title_document": "C of O", 
"survey_status": "Completed",
                "features": ["CBD Location", "C of O", "High Traffic", "Metro 
Access"], 
                "description": "Prime commercial land in the heart of Abuja's 
business district", 
                "owner_contact": "+234-123-456-804", "status": "active", 
"uploaded_date": "2024-02-05", 
                "demand_score": 8.9, "competition_score": 6.5, 
"seasonal_multiplier": 1.2, 
                "photos": 14, "description_quality": 9.1, "inquiry_rate": 0.87, 
"avg_rating": 4.7,
                "owner_id": "land_owner_5", "available_from": "2024-02-20", 
"zoning": "Commercial",
                "access_road": "Tarred", "topography": "Flat", "utilities": 
["Electricity", "Water", "Internet"]
            }
...
def show_login_signup():
    ...
    with tab2:
        ...
        user_type = st.selectbox("I am a:", ["tenant", "host", "agent", 
"investor", "land_developer"])
        
        agree_terms = st.checkbox("I agree to the Terms of Service and Privacy 
Policy")
...
def show_portal_selection():
    ...
    with col2:
        ...
        if st.button("🏠 Properties Portal", key="properties_portal", 
use_container_width=True):
        ...
        # Land Portal Button
        if st.button("🌍 Land Portal", key="land_portal", 
use_container_width=True):
...
def show_mr_x_chat():
    ...
    st.markdown("*Your intelligent guide powered by knowledge database*")
        ...
        if st.button("🔧 Test Database Connection"):
            with st.spinner("Testing connection..."):
                if st.session_state.ai_system.knowledge_base.connect():
                    st.success("✅ CSV Database Connected!")
                else:
                    st.error("❌ CSV Database Connection Failed")
    
    with col2:
        user_type = st.selectbox("I am a:", ["Guest/Buyer", "Property Host/Owner", 
"Real Estate Agent", "Property Investor", "Tenant"])
    with col3:
    ...
    with chat_container:
        if not st.session_state.mr_x_chat_history:
            welcome_msg = st.session_state.ai_system.mr_x_response("", 
{"user_type": user_type.lower().split('/')[0]})
    ...
    user_input = st.text_input("Ask MR X anything about properties...", 
                              placeholder="e.g., 'Find me investment properties in 
Lekki' or 'What's the best area for families in Abuja?'",
                              key="mr_x_input")
    ...
    quick_actions = {
        "Investment Analysis": "Analyze property investment potential in my 
budget",
        "Neighborhood Guide": "Tell me about the best neighborhoods for my 
lifestyle",
        "Market Trends": "What are current property market trends in Lagos and 
Abuja?",
    ...
    for i, (action, prompt) in enumerate(quick_actions.items()):
        ...
        with col:
            if st.button(action, key=f"property_action_{i}"):
                context = {"properties": get_all_properties(), "user_type": 
user_type}
                response = st.session_state.ai_system.mr_x_response(prompt, 
context)
...
def show_landlord_chat():
    ...
    st.markdown("*Your strategic advisor powered by knowledge database*")
    ...
    with col2:
        user_type = st.selectbox("I am a:", ["Guest/Investor", "Land Developer", 
"Real Estate Agent", "Land Owner", "Investment Firm"], key="landlord_user_type")
    with col3:
    ...
    with chat_container:
        if not st.session_state.landlord_chat_history:
            welcome_msg = st.session_state.ai_system.landlord_response("", 
{"user_type": user_type.lower().split('/')[0]})
    ...
    user_input = st.text_input("Ask LANDLORD anything about land investment...", 
                              placeholder="e.g., 'Find commercial land in Lagos' 
or 'Analyze development potential in Maitama'",
                              key="landlord_input")
    
    if st.button("Send Message", type="primary", key="send_landlord") and 
user_input:
        context = {
        ...
        response = st.session_state.ai_system.landlord_response(user_input, 
context)
    ...
    quick_actions = {
        "Investment Analysis": "Analyze land investment opportunities in my budget 
range",
    ...
    for i, (action, prompt) in enumerate(quick_actions.items()):
        ...
        with col:
            if st.button(action, key=f"land_action_{i}"):
                ...
                response = st.session_state.ai_system.landlord_response(prompt, 
context)
...
def show_property_search():
    ...
    with col2:
        avg_rent = np.mean([p.get('rent_monthly', 0) for p in all_props if 
p.get('rent_monthly')])
    ...
    with col4:
        recent_count = len([p for p in all_props if p.get("uploaded_date", "") >= 
"2024-02-01"])
        st.metric("New Listings", recent_count)
    
    search_mode = st.radio("Search Mode:", ["Quick Filter Search", "Chat with MR 
X"])
    ...
    if search_mode == "Quick Filter Search":
        with st.expander("Search Filters", expanded=True):
            ...
            with col1:
                ...
                filter_type = st.selectbox("Type", ["All", "Rent", "Sale", 
"Both"])
            with col2:
                filter_beds = st.selectbox("Bedrooms", ["All", "1", "2", "3", "4", 
"5+"])
                max_budget = st.number_input("Max Budget (₦)", min_value=0, 
value=0, help="0 = no limit")
            with col3:
                min_area = st.number_input("Min Area (sqm)", min_value=0, value=0)
                location_filter = st.selectbox("Location", ["All", "Ikoyi", "Lekki 
Phase 1", "Yaba", "Maitama", "Wuse II"])
            with col4:
                required_amenities = st.multiselect("Must Have", ["Security", 
"Generator", "Pool", "Gym", "Parking", "Garden"])
        ...
        if st.button("Search Properties", type="primary"):
            ...
            if location_filter != "All":
                filtered = [p for p in filtered if p.get("location") == 
location_filter]
...
target_beds]
            ...
            if max_budget > 0:
                filtered = [p for p in filtered if 
                          (p.get("rent_monthly") and p["rent_monthly"] <= 
max_budget) or
                          (p.get("sale_price") and p["sale_price"] <= max_budget)]
            
            if min_area > 0:
                filtered = [p for p in filtered if p.get("area_sqm", 0) >= 
min_area]
            
            if required_amenities:
                filtered = [p for p in filtered if all(amenity in 
p.get("amenities", []) for amenity in required_amenities)]
...
def show_land_search():
    ...
    st.markdown("*AI-powered land discovery with intelligent investment 
matching*")
    ...
    with col2:
        avg_price = np.mean([l.get('price_per_sqm', 0) for l in all_land if 
l.get('price_per_sqm')])
    ...
    with col4:
        recent_count = len([l for l in all_land if l.get("uploaded_date", "") >= 
"2024-02-01"])
        st.metric("New Listings", recent_count)
    
    search_mode = st.radio("Search Mode:", ["Quick Filter Search", "Chat with 
LANDLORD"])
    ...
    if search_mode == "Quick Filter Search":
        with st.expander("Search Filters", expanded=True):
            ...
            with col1:
                filter_city = st.selectbox("City", ["All", "Lagos", "Abuja"], 
key="land_city")
                land_use_filter = st.selectbox("Land Use", ["All", "Residential", 
"Commercial", "Industrial"])
            with col2:
                min_size = st.number_input("Min Size (sqm)", min_value=0, value=0)
                max_budget = st.number_input("Max Budget (₦)", min_value=0, 
value=0, key="land_budget")
            with col3:
                location_filter = st.selectbox("Location", ["All", "Victoria 
Island", "Lekki Phase 1", "Maitama", "Ikorodu"], key="land_location")
                title_filter = st.selectbox("Title Document", ["All", "C of O", 
"Deed of Assignment"])
            with col4:
                required_features = st.multiselect("Must Have", ["C of O", "Survey 
Plan", "Corner Piece", "Waterfront", "Highway Access"])
        ...
        if st.button("Search Land Plots", type="primary"):
            ...
            if location_filter != "All":
                filtered = [l for l in filtered if l.get("location") == 
location_filter]
            
            if land_use_filter != "All":
                filtered = [l for l in filtered if l.get("land_use") == 
land_use_filter]
            
            if title_filter != "All":
                filtered = [l for l in filtered if l.get("title_document") == 
title_filter]
            
            if max_budget > 0:
                filtered = [l for l in filtered if l.get("price_total", 0) <= 
max_budget]
            
            if min_size > 0:
                filtered = [l for l in filtered if l.get("land_size_sqm", 0) >= 
min_size]
            
            if required_features:
                filtered = [l for l in filtered if all(feature in 
l.get("features", []) for feature in required_features)]
    ...
    for i, prop in enumerate(properties, 1):
        with st.container():
            ...
            with col1:
                ...
                getattr(st, indicator_color)(f"**{i}. {prop['name']}** - 
{value_indicator}")
                
                st.markdown(f"""
                **Location:** {prop['location']}, {prop.get('city', '')}
                
                **Details:** {prop['bedrooms']} bed • {prop['bathrooms']} bath • 
{prop['area_sqm']} sqm • Built {prop.get('year_built', 'N/A')}
                
                **Price:** {'Rent: ₦' + f"{prop['rent_monthly']:,}/month" if 
prop.get('rent_monthly') else ''} {'Sale: ₦' + f"{prop['sale_price']:,}" if 
prop.get('sale_price') else ''}
                
                **Quality:** Rating: {prop.get('avg_rating', 4.0)}/5 • Demand 
Score: {prop.get('demand_score', 7.0)}/10
            ...
            with col2:
                ...
                if st.button(f"Contact Owner", 
key=f"contact_property_{prop['id']}"):
                    st.success("Contact initiated!")
                
                if st.button(f"Save to Favorites", 
key=f"save_property_{prop['id']}"):
    ...
    for i, land in enumerate(land_plots, 1):
        with st.container():
            ...
            with col1:
                ...
                getattr(st, indicator_color)(f"**{i}. {land['name']}** - 
{value_indicator}")
                
                st.markdown(f"""
                **Location:** {land['location']}, {land.get('city', '')}
                
                **Details:** {land['land_size_sqm']:,} sqm • {land['land_use']} • 
{land.get('zoning', 'N/A')} Zone
                
                **Price:** ₦{land['price_total']:,} (₦{land.get('price_per_sqm', 
0):,}/sqm)
                
                **Documentation:** {land.get('title_document', 'N/A')} • Survey: 
{land.get('survey_status', 'N/A')}
                
                **Infrastructure:** Road: {land.get('access_road', 'N/A')} • 
Utilities: {', '.join(land.get('utilities', []))}
...
def show_property_upload_form():
    ...
    with st.form("enhanced_property_upload"):
        ...
        with col1:
            property_name = st.text_input("Property Title*", placeholder="e.g., 
Luxury 3BR Waterfront Apartment")
            city = st.selectbox("City*", ["Lagos", "Abuja"])
            location = st.text_input("Specific Location*", placeholder="e.g., 
Ikoyi, Lekki Phase 1")
...
value=2)
        ...
        with col2:
            area_sqm = st.number_input("Area (Square Meters)*", min_value=20.0, 
value=120.0)
            year_built = st.number_input("Year Built*", min_value=1950, 
max_value=2024, value=2020)
            property_type = st.selectbox("Property Type*", ["Apartment", "Duplex", 
"Bungalow", "Mansion", "Penthouse"])
            furnished = st.selectbox("Furnishing", ["Unfurnished", 
"Semi-Furnished", "Fully Furnished"])
        ...
        with col1:
            rent_monthly = st.number_input("Monthly Rent (₦)", min_value=0, 
value=0, help="Leave as 0 if not for rent")
        
        with col2:
            sale_price = st.number_input("Sale Price (₦)", min_value=0, value=0, 
help="Leave as 0 if not for sale")
        ...
        selected_amenities = st.multiselect("Select available amenities", 
amenity_options)
        
        st.subheader("Property Description")
        description = st.text_area("Property Description*", placeholder="Describe 
your property's unique features...", height=100)
        
        st.subheader("Contact Information")
        col1, col2 = st.columns(2)
        
        with col1:
            owner_contact = st.text_input("Phone Number*", 
placeholder="+234-XXX-XXX-XXXX")
            owner_email = st.text_input("Email Address", 
placeholder="your.email@example.com")
        
        with col2:
            whatsapp_number = st.text_input("WhatsApp Number", placeholder="For 
quick communication")
            preferred_contact = st.selectbox("Preferred Contact Method", ["Phone 
Call", "WhatsApp", "Email", "Any"])
        
        st.subheader("Legal & Compliance")
        property_documents = st.checkbox("I confirm all property documents are 
valid and up-to-date")
        marketing_consent = st.checkbox("I consent to marketing this property on 
RealtyXperience platform")
        terms_agreement = st.checkbox("I agree to the Terms of Service and Privacy 
Policy")
        
        submitted = st.form_submit_button("List Property", type="primary")
        
        if submitted:
            required_fields = [property_name, city, location, bedrooms, bathrooms, 
owner_contact, description]
            if not all(required_fields):
                st.error("Please fill in all required fields marked with *")
            elif rent_monthly == 0 and sale_price == 0:
                st.error("Please specify either rental price or sale price (or 
both)")
            elif not all([property_documents, marketing_consent, 
terms_agreement]):
                ...
                property_data = {
                    ...
                    "furnished": furnished,
                    "rent_monthly": int(rent_monthly) if rent_monthly > 0 else 
None,
...
def show_land_upload_form():
    ...
    st.markdown("*Get AI-powered insights and investment optimization 
recommendations*")
    ...
    with st.form("enhanced_land_upload"):
        ...
        with col1:
            land_name = st.text_input("Land Title*", placeholder="e.g., Prime 
Commercial Waterfront Land")
            city = st.selectbox("City*", ["Lagos", "Abuja"], 
key="land_upload_city")
            location = st.text_input("Specific Location*", placeholder="e.g., 
Victoria Island, Maitama")
            land_size_sqm = st.number_input("Land Size (Square Meters)*", 
min_value=100.0, value=2000.0)
            land_use = st.selectbox("Intended Use*", ["Residential", "Commercial", 
"Industrial", "Mixed Use"])
        
        with col2:
            zoning = st.selectbox("Zoning Classification", ["Residential", 
"Commercial", "Industrial", "Mixed", "Agricultural"])
            topography = st.selectbox("Topography", ["Flat", "Sloped", "Hilly", 
"Waterfront", "Mixed"])
            access_road = st.selectbox("Road Access", ["Tarred", "Gravel", 
"Highway", "Expressway", "Untarred"])
            drainage = st.selectbox("Drainage", ["Excellent", "Good", "Fair", 
"Poor", "Requires Work"])
        ...
        with col1:
            price_total = st.number_input("Total Price (₦)*", min_value=1000000, 
value=50000000)
        
        with col2:
            price_per_sqm = st.number_input("Price per SqM (₦)", min_value=0, 
value=int(price_total/land_size_sqm) if land_size_sqm > 0 else 0, 
                                          help="Auto-calculated, but you can 
adjust")
        ...
        with col1:
            title_document = st.selectbox("Title Document*", ["Certificate of 
Occupancy (C of O)", "Deed of Assignment", "Customary Right", "Survey Plan Only"])
            survey_status = st.selectbox("Survey Status*", ["Completed", "In 
Progress", "Required", "Not Available"])
        
        with col2:
            government_consent = st.selectbox("Government Consent", ["Available", 
"In Process", "Required", "Not Required"])
            excision_status = st.selectbox("Excision Status", ["Excised", 
"Gazette", "Not Excised", "In Process"])
        
        st.subheader("Infrastructure & Utilities")
        utility_options = ["Electricity", "Water", "Gas", "Internet", "Sewage", 
"Phone Lines"]
        available_utilities = st.multiselect("Available Utilities", 
utility_options)
        ...
        description = st.text_area("Land Description*", placeholder="Describe your 
land's unique investment potential...", height=100)
        ...
        with col1:
            owner_contact = st.text_input("Phone Number*", 
placeholder="+234-XXX-XXX-XXXX", key="land_contact")
            owner_email = st.text_input("Email Address", 
placeholder="your.email@example.com", key="land_email")
        
        with col2:
            whatsapp_number = st.text_input("WhatsApp Number", placeholder="For 
quick communication", key="land_whatsapp")
            preferred_contact = st.selectbox("Preferred Contact Method", ["Phone 
Call", "WhatsApp", "Email", "Any"], key="land_contact_pref")
        
        st.subheader("Legal & Compliance")
        land_documents = st.checkbox("I confirm all land documents are valid and 
up-to-date")
        marketing_consent = st.checkbox("I consent to marketing this land on 
RealtyXperience platform", key="land_marketing")
        terms_agreement = st.checkbox("I agree to the Terms of Service and Privacy 
Policy", key="land_terms")
        
        submitted = st.form_submit_button("List Land", type="primary")
        
        if submitted:
            required_fields = [land_name, city, location, land_size_sqm, 
price_total, owner_contact, description]
            if not all(required_fields):
                st.error("Please fill in all required fields marked with *")
...
{})
    ...
    st.title(f"Welcome back, {user_data.get('full_name', 
...
{prop['area_sqm']} sqm
        ...
        st.info("No properties listed yet. Navigate to 'List New Property' to add 
your first property")
...
def show_land_developer_dashboard():
    ...
    if user_land:
        ...
        for land in user_land:
            with st.expander(f"{land['name']} - {land['location']}", 
expanded=False):
                st.markdown(f"""
                **Size:** {land['land_size_sqm']:,} sqm
                **Use:** {land['land_use']}
                **Status:** {land.get('status', 'active').title()}
                **Rating:** {land.get('avg_rating', 4.0)}/5
                """)
    else:
        st.info("No land plots listed yet. Navigate to 'List New Land' to add your 
first land")
...
def main_app():
    ...
    with st.sidebar:
        ...
        if st.button("Test CSV DB", key="sidebar_db_test"):
            if st.session_state.ai_system.knowledge_base.connect():
                st.success("✅ CSV Database: Connected")
            else:
                st.error("❌ CSV Database: Offline")
        
        user_type = st.session_state.user_type
        
        if portal == "properties":
            if user_type == "host":
                page_options = ["Dashboard", "List New Property", "MR X AI 
Assistant"]
            else:
                page_options = ["Dashboard", "Find Properties", "MR X AI 
Assistant", "List Property"]
        
        elif portal == "land":
            if user_type == "land_developer":
                page_options = ["Dashboard", "List New Land", "LANDLORD AI 
Assistant"]
            else:
                page_options = ["Dashboard", "Find Land", "LANDLORD AI Assistant", 
"List Land"]
        ...
        if portal == "properties":
            all_props = get_all_properties()
            if all_props:
                avg_demand = np.mean([p.get('demand_score', 7.0) for p in 
all_props])
                market_status = "High Demand" if avg_demand > 8 else "Stable"
                st.info(f"Market: {market_status}")
        
        elif portal == "land":
            all_land = get_all_land()
            if all_land:
                avg_demand = np.mean([l.get('demand_score', 7.0) for l in 
all_land])
                market_status = "High Demand" if avg_demand > 8 else "Stable"
                st.info(f"Market: {market_status}")
