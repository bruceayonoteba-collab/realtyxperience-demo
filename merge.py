...
class SnowflakeKnowledgeBase:
    """Connects to your Snowflake knowledge database"""
    ...
    def search_property_knowledge(self, query, max_results=3):
        ...
        try:
            ...
            WHERE LOWER(question || ' ' || answer || ' ' || tags) LIKE LOWER(%s)
            ORDER BY 
                CASE 
                    WHEN LOWER(question) LIKE LOWER(%s) THEN 1
                    WHEN LOWER(category) LIKE LOWER(%s) THEN 2
                    ELSE 3
                END
            LIMIT %s
            """
            ...
            st.error(f"Error searching PROPERTY_KNOWLEDGE: {e}")
            return []
    ...
    def search_land_knowledge(self, query, max_results=3):
        ...
        try:
            ...
            st.error(f"Error searching LAND_KNOWLEDGE: {e}")
            return []
...
class RealtyXperienceAI:
    ...
    def __init__(self):
        self.knowledge_base = SnowflakeKnowledgeBase()
        try:
